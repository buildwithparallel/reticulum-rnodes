#line 1 "/Users/refactored/Code/Ad_Hoc/RNode_Firmware_CE/Bluetooth.h"
...
char bt_devname[11];
    ...
    void bt_authentication_complete_callback(ble_gap_conn_desc *desc) {
      // NimBLE provides security state via the connection descriptor.
    ...
    bool bt_setup_hw() {
      ...
      if (!bt_ready) {
        ...
        esp_read_mac(dev_bt_mac, ESP_MAC_WIFI_STA);
        char *data = (char*)malloc(BT_DEV_ADDR_LEN+1);
        for (int i = 0; i < BT_DEV_ADDR_LEN; i++) {
            data[i] = dev_bt_mac[i];
        }
    ...
    void bt_security_setup() {
      ...
      BLESecurity::setKeySize(key_size);
      BLESecurity::setInitEncryptionKey(init_key);
      BLESecurity::setRespEncryptionKey(rsp_key);
      #endif
    }
