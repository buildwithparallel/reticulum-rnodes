#line 1 "/Users/refactored/Code/Ad_Hoc/RNode_Firmware_CE/Display.h"
...
bool display_init() {
    ...
    #else
    // SSD1306 OLEDs are commonly found at 0x3C or 0x3D.
    // Some LoRa32 v1.x batches ship with 0x3D, so try both.
    bool display_ok = display.begin(SSD1306_SWITCHCAPVCC, display_address);
    if (!display_ok && (display_address == 0x3C || display_address == 0x3D)) {
      uint8_t alt_addr = (display_address == 0x3C) ? 0x3D : 0x3C;
      display_ok = display.begin(SSD1306_SWITCHCAPVCC, alt_addr);
      if (display_ok) { display_address = alt_addr; }
    }
    if (!display_ok) {
      return false;
    } else {
