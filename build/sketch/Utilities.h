#line 1 "/Users/refactored/Code/Ad_Hoc/RNode_Firmware_CE/Utilities.h"
...
uint8_t boot_vector = 0x00;
  ...
  // Keep NeoPixel lit after activity (RX/TX)
  uint32_t np_activity_until = 0;
  ...
  void np_set_activity(uint32_t hold_ms, uint8_t r, uint8_t g, uint8_t b) {
    np_activity_until = millis() + hold_ms;
    npset(r, g, b);
  }

  void np_update() {
    if (np_activity_until != 0 && (int32_t)(millis() - np_activity_until) >= 0) {
      np_activity_until = 0;
      npset(0, 0, 0);
    }
  }
...
#if MCU_VARIANT == MCU_ESP32
	#if HAS_NP == true
		void led_rx_on()  { np_set_activity(NP_RX_TIMEOUT_MS, NP_RX_R, NP_RX_G, NP_RX_B); }
		void led_rx_off() { /* keep lit until timeout */ }
		void led_tx_on()  { np_set_activity(NP_TX_TIMEOUT_MS, NP_TX_R, NP_TX_G, NP_TX_B); }
		void led_tx_off() { /* keep lit until timeout */ }
